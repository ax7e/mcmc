...
# %%
def GameMCMC12(Y, X, year, country, covset, m=10000, burnin=1, h=0.001):
    N = len(Y)
    ...
    m = 1000 # Replace this with your value for m
    ...
    for g in range(1, m+burnin+1):
        ...
        if g > burnin:
            gg = g-burnin
            betaUSReward[gg,:] = betaucr
            betaUSPunish[gg,:] = betauncp
            betaRComply[gg,:] = betarc
            betaRNComply[gg,:] = betarnc
            betaCIncrease[gg,:] = betaci
            probmedia = np.hstack((p1sm, p3sm, p5sm, p7sm, p12sm))
            probabilityMedian[gg,:] = probmedia
            probsd = np.hstack((p1sd, p3sd, p5sd, p7sd, p12sd))


GameMCMC12(Y=Y82, X=X7, year=year, country=country, covset=collistDA, m=10000, burnin=5000, h=0.01) 